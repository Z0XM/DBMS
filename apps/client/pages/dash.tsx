...
export default function Dash({
	...
	const reloadChatData = () => {
		axios
			.get(`http://localhost:4000/chat?user1=${userData.username}&user2=${chatUsername}`)
			.then((res) => {
	...
	const deletePost = (postid: number) => {
		axios
			.delete(`http://localhost:4000/post?post_id=${postid}`)
			.then((res) => {
				!res.data.error && setPosts(res.data.data);
			})
			.catch(console.error);
									...
									return (
										<div
											...
											<div style={{ fontSize: '1.2rem', color: '#e0115f' }}>
												{post.username_fk != userData.username ? (
													<span
														style={{ cursor: 'pointer' }}
														onClick={() => {
															setChatUsername(post.username_fk);
														}}>
														{post.username_fk}
													</span>
												) : (
										...
										return (
											<div
												...
												style={{
													...
													width: '60%',
													marginLeft:
														chat.s_username_fk == userData.username
															? '40%'
															: '0'
												}}>
												<div
